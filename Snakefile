...
(outName, sampSheetDate) = os.path.basename(input_sample_sheet)[:-4].split(
    "_AnalysisManifest_"
)
...
ylimDict = {
    "_start": ("0", "100"),
    "_filter1": (str(samp_cr_1 * 100), "100"),
    "_filter2": (str(samp_cr_2 * 100), "100"),
}
...
def makeFamDict(ibd_csv):
    ...
    with open(ibd_csv) as f:
        ...
        while line != "":
            ...
            if (
                famDict.get(sub1)
                and famDict.get(sub2)
                and (famDict[sub1] != famDict[sub2])
            ):
                ...
                for sub in famDict.keys():
                    if (
                        ...
                        and famDict[sub] != fam
                        and (
                            famDict[sub] == famDict[sub1]
                            or famDict[sub] == famDict[sub2]
                        )
                    ):
                        famDict[sub] = fam
...
def MakeRelatedDict(
    ...
    sample_to_sub_file,
    sub_fam_file,
    relatedThresh=float(config["pi_hat_threshold"]),
):
...
def makeSubjectToSampListDict(SampleSheet):
    ...
    with codecs.open(SampleSheet, "r", encoding="utf-8", errors="ignore") as f:
        ...
        if subjectIdCol == None or sampGroupCol == None or limsIdCol == None:
            print(
                "Subject ID or Sample_Group or LIMS_Individual_ID not found in sample sheet"
            )
            sys.exit(1)
        line = f.readline()
        while line != "":
    ...
    with codecs.open(SampleSheet, "r", encoding="utf-8", errors="ignore") as f:
        ...
        while line != "":
            if line.strip():
                ...
                if os.path.isfile(idatBase + "_Red.idat") and os.path.isfile(
                    idatBase + "_Grn.idat"
                ):
...
(
    ...
    gtcFiles,
    chipIdToSampDict,
) = makeChipIdToSampDict(sample_sheet, gtc_dir)
...
if skip_contam_check == "FALSE":
    ...
    include: "modules/Snakefile_ped_preprocess" #include: 'modules/Snakefile_plink_contam'
    ...
    include: "modules/Snakefile_bed_preprocess" #include: 'modules/Snakefile_plink_contam'
