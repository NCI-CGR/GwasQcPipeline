...
@app.command()
def main(qc_table: Path, eigenvec: Path, population: str, outfile_case: Path, outfile_ancestry: Path):
    pca = (
        Eigenvec(eigenvec)
        .components.join(
            subject_qc_table.read(qc_table).set_index("Group_By_Subject_ID"), how="left"
        )
        .reindex(["Ancestry","case_control", "PC1", "PC2", "PC3", "PC4", "PC5", "PC6"], axis=1)
        .apply(_update_categories)
    )

    g_case = plot(pca, population)
    g_ancestry = plot_a(pca, population)

    if outfile_case:
        g_case.savefig(outfile_case)
        g_ancestry.savefig(outfile_ancestry)

#plot pca colored by case/control status
...
# plot pca colored by ancestry
def plot_a(df: pd.DataFrame, population: str) -> sns.PairGrid:
    sns.set_context("paper")  # use seaborn's context to make sane plot defaults for a paper
    
    g = sns.PairGrid(df, hue="Ancestry", corner=True)
    g.map_lower(sns.scatterplot, s=10, alpha=0.8, linewidth=0)
    g.map_diag(sns.kdeplot)
    g.add_legend(
        bbox_to_anchor=(0.2, 0.9), loc=2, prop={"size": "large"}, title_fontsize="xx-large"
