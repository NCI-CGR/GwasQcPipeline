...
def getNumSNPs(illumina_manifest_file):
    ...
    subprocess.check_output(
        ["glu", "convert.dump_bpm", illumina_manifest_file, "-o", "illumina_manifest_file.txt"]
    )
...
def get_args():
    ...
    oneOfTheseRequired.add_argument(
        "-p",
        "--path_to_plink_file",
        type=str,
        required=False,
        help="Full path to either PLINK ped or bed to use as input.\n\
    ...
    requiredArgs.add_argument(
        ...
        type=str,
        help="REQUIRED. Full path to the base directory for the Gwas QC pipeline output.  Defaults to /DCEG/CGF/GWAS/Scans/GSA_Lab_QC/Manifest#/builds/QC_v#_date",
    )
    requiredWithDefaults.add_argument(
        "--snp_cr_1",
        type=float,
        default=0.80,
        help="REQUIRED. SNP call rate filter 1.  default= 0.80",
    )
    requiredWithDefaults.add_argument(
        "--samp_cr_1",
        type=float,
        default=0.80,
        help="REQUIRED. Sample call rate filter 1.  default= 0.80",
    )
    requiredWithDefaults.add_argument(
        "--snp_cr_2",
        type=float,
        default=0.95,
        help="REQUIRED. SNP call rate filter 2.  default= 0.95",
    )
    requiredWithDefaults.add_argument(
        "--samp_cr_2",
        type=float,
        default=0.95,
        help="REQUIRED. Sample call rate filter 2.  default= 0.95",
    )
    requiredWithDefaults.add_argument(
        "--ld_prune_r2",
        type=float,
        default=0.10,
        help="REQUIRED. r-squared cutoff for ld pruning of SNPs to use for IBD and concordance.  default= 0.10",
    )
    requiredWithDefaults.add_argument(
        "--maf_for_ibd",
        type=float,
        default=0.20,
        help="REQUIRED. MAF cutoff of SNPs to use for IBD and concordance.  default= 0.20",
    )
    requiredArgs.add_argument(
        "-s",
        "--sample_sheet",
        type=str,
        required=True,
        help="Full path to illimina style sample sheet csv file.",
    )
    requiredArgs.add_argument(
        "--subject_id_to_use",
        type=str,
        default="LIMS_Individual_ID",
        help="Name of column in sample sheet that corresponds to subject ID to use.",
    )  ##I should be able to add a default once this is available
    requiredWithDefaults.add_argument(
        "--ibd_pi_hat_cutoff",
        type=float,
        default=0.95,
        help="REQUIRED. PI_HAT cutoff to call samples replicates.  default= 0.95",
    )  ##this can be deleted if just using SNP concordance
    requiredWithDefaults.add_argument(
        "--dup_concordance_cutoff",
        type=float,
        default=0.95,
        help="REQUIRED. SNP concordance cutoff to call samples replicates.  default= 0.95",
    )
    requiredWithDefaults.add_argument(
        "--lims_output_dir",
        type=str,
        default="/DCEG/CGF/Laboratory/LIMS/drop-box-prod/gwas_primaryqc",
        help="Directory to copy QC file to upload to LIMS",
    )
    requiredWithDefaults.add_argument(
        "--contam_threshold",
        type=float,
        default=0.10,
        help="REQUIRED. Cutoff to call a sample contaminated.  default= 0.10",
    )
    requiredWithDefaults.add_argument(
        "--contam_pop",
        type=str,
        default="AF",
        choices=["AF", "EAS_AF", "AMR_AF", "AFR_AF", "EUR_AF", "SAS_AF"],
        help="REQUIRED. 1KG pop to use for freq for contam test. Must be one of:  AF,EAS_AF,AMR_AF,AFR_AF,EUR_AF,SAS_AF.  default= AF",
    )
    parser.add_argument(
        "-i",
        "--illumina_manifest_file",
        type=str,
        default="/DCEG/CGF/Infinium/Resources/Manifests/GSAMD-Files/build37/GSAMD-24v1-0_20011747_A1.bpm",
        help="Full path to illimina .bpm manifest file. Required for gtc files.",
    )
    requiredWithDefaults.add_argument(
        "--strand",
        type=str,
        default="TOP",
        choices=["TOP", "FWD"],
        help="REQUIRED. strand to use for genotypes. Must be one of:  TOP, FWD.  default= TOP",
    )
    parser.add_argument(
        "-a", "--adpc_file", type=str, help="Full path to adpc.bin file. Required for PLINK input."
    )
    parser.add_argument(
        "-g",
        "--gtc_dir",
        type=str,
        help="Full path to gtc directory to use instead of project directory, which is the default.  Will recursively find gtc files in this directory.",
    )
    requiredArgs.add_argument(
        "--expected_sex_col_name",
        type=str,
        default="Expected_Sex",
        help="Name of column in sample sheet that corresponds to expected sex of sample.",
    )  ##I should be able to add a default once this is available
    requiredWithDefaults.add_argument(
        "-q",
        "--queue",
        type=str,
        default="all.q,seq-alignment.q,seq-calling.q,seq-calling2.q,seq-gvcf.q,bigmem.q",
        help='OPTIONAL. Queue on cgemsiii to use to submit jobs.  Defaults to all of the seq queues and all.q if not supplied.  default="all.q,seq-alignment.q,seq-calling.q,seq-calling2.q,seq-gvcf.q,bigmem.q"',
    )
    requiredWithDefaults.add_argument(
        "--remove_contam",
        type=str,
        default="YES",
        help='REQUIRED. If "YES" contaminated samples will be removed prior to sample to subject transformation.  Defaults to "YES"',
    )
    ...
    requiredWithDefaults.add_argument(
        ...
        type=str,
        default="FALSE",
        help='REQUIRED. If "YES" contamination check will be skipped.  Defaults to "FALSE" when start with gtc files. Must be "YES" when start with plink files.',
    )
    requiredWithDefaults.add_argument(
        "--skip_intensity_check",
        type=str,
        default="FALSE",
        help='REQUIRED. If "YES" intensity check will be skipped.  Defaults to "FALSE" when start with gtc files. Must be "YES" when start with plink files.',
    )
    requiredWithDefaults.add_argument(
        "--remove_sex_discordant",
        type=str,
        default="YES",
        help='REQUIRED. If "YES" sex discordant samples will be removed prior to sample to subject transformation.  Defaults to "YES"',
    )
    requiredWithDefaults.add_argument(
        "--remove_rep_discordant",
        type=str,
        default="YES",
        help='REQUIRED. If "YES" known replicate discordant samples will be removed prior to sample to subject transformation.  Defaults to "YES"',
    )
    requiredWithDefaults.add_argument(
        "--remove_unexpected_rep",
        type=str,
        default="YES",
        help='REQUIRED. If "YES" all unexpected replicate samples will be removed prior to sample to subject transformation.  Defaults to "YES"',
    )
    requiredWithDefaults.add_argument(
        "--pi_hat_threshold",
        type=float,
        default=0.20,
        help="REQUIRED. PI_HAT cutoff to call subjects related.  default= 0.20, to remove 2nd degree relatives or higher.",
    )
    requiredWithDefaults.add_argument(
        "--minimum_pop_subjects",
        type=int,
        default=50,
        help="REQUIRED. Number of subjects needed in order to analyze a population.  default= 50.",
    )
    requiredWithDefaults.add_argument(
        "--control_hwp_thresh",
        type=int,
        default=50,
        help="REQUIRED. Number of controls needed in order to just use controls for HWP.  default= 50.",
    )
    requiredWithDefaults.add_argument(
        "--autosomal_het_thresh",
        type=float,
        default=0.10,
        help="REQUIRED. F coefficient from autosomal heterozygosity check cutoff for subject removal.  default= 0.10",
    )
    requiredWithDefaults.add_argument(
        "-w",
        "--word_doc_template",
        type=str,
        default="/DCEG/CGF/Bioinformatics/Production/Eric/refFiles/GwasQcPipeline_Template_interim.docx",
        help="REQUIRED. Template to use to generate QC report word docx",
    )
    parser.add_argument(
        "-u",
        "--unlock_snakemake",
        action="store_true",
        help="OPTIONAL. If pipeline was killed unexpectedly you may need this flag to rerun",
    )
    parser.add_argument(
        "-f",
        "--finish",
        action="store_true",
        help="OPTIONAL. Use with -d option to restart pipeline or update with new features without making new config file, etc.",
    )
...
def main():
    ...
    if sampSheetOwner != "cgflims":
        print(
            "WARNING: AnalysisManifest is not generated by LIMS.  File owner is " + sampSheetOwner
        )
    ...
    if not args.finish:
        ...
        makeConfig(
            ...
            args.maf_for_ibd,
            args.sample_sheet,
            args.subject_id_to_use,
            args.ibd_pi_hat_cutoff,
            args.dup_concordance_cutoff,
            args.illumina_manifest_file,
            args.expected_sex_col_name,
            numSamps,
            args.lims_output_dir,
            args.contam_threshold,
            args.adpc_file,
            args.gtc_dir,
            args.remove_contam,
            args.remove_sex_discordant,
            args.remove_rep_discordant,
            args.remove_unexpected_rep,
            args.pi_hat_threshold,
            args.autosomal_het_thresh,
            args.minimum_pop_subjects,
            args.control_hwp_thresh,
            args.word_doc_template,
            args.contam_pop,
            args.strand,
            numSNPs,
            args.skip_contam_check,
            args.skip_intensity_check,
        )
    ...
    qsubTxt += (
        'snakemake --rerun-incomplete --cluster "qsub -V -q {cluster.q} -pe by_node {threads} '
    )
    qsubTxt += (
        "-o "
        + outDir
        + "/logs/ -e "
        + outDir
        + '/logs/" --jobs 4000 --cluster-config cluster.yaml --latency-wait 300\n'
    )
    makeQsub(outDir + "/GwasQcPipeline.sh", qsubTxt)
    runQsub(
        outDir + "/GwasQcPipeline.sh",
        os.path.basename(outDir),
        "seq-alignment.q,seq-calling.q,seq-calling2.q,seq-gvcf.q",
    )
    print(
        "GWAS QC Pipeline submitted.  You should receive an email when the pipeline starts and when it completes."
    )
