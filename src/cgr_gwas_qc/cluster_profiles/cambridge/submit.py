...
@dataclass
class CambridgeOptions(ClusterOptions):
    queue: Set[str] = field(default_factory=lambda: {"cclake", "cclake"})
...
def main(jobscript=None):
    ...
    cambridge_options = CambridgeOptions.from_dict(options)

    # Submit job script
    cmd = cambridge_options.command + [jobscript]
