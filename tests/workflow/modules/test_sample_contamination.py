...
def test_agg_median_idat_intensity(tmp_path, conda_envs):
    ...
    data_store = (
        ...
        .make_snakefile(
            ...
            rule all:
                input: "sample_contamination/agg_median_idat_intensity.csv"
            """
        )
    )
    ...
    for r in data_store.ss.data.itertuples(index=False):
        obs_file = (
            tmp_path
            / f"sample_contamination/median_idat_intensity/{r.Sample_ID}.{r.SentrixBarcode_A}.{r.SentrixPosition_A}.csv"
        )
    ...
    obs_table = pd.read_csv(tmp_path / "sample_contamination/agg_median_idat_intensity.csv")
...
def test_convert_gtc_to_illumina_adpc(tmp_path):
    ...
    data_store = (
        ...
        .make_snakefile(
            ...
            rule all:
                input:
                    cfg.expand("sample_contamination/convert_gtc_to_illumina_adpc/{Sample_ID}.adpc.bin")
                    + cfg.expand("sample_contamination/convert_gtc_to_illumina_adpc/{Sample_ID}.adpc.bin.numSnps.txt")
            """
        )
    )
    ...
    for r in data_store.ss.data.itertuples(index=False):
        obs_adpc = (
            tmp_path / f"sample_contamination/convert_gtc_to_illumina_adpc/{r.Sample_ID}.adpc.bin"
        )
        ...
        obs_counts = (
            tmp_path
            / f"sample_contamination/convert_gtc_to_illumina_adpc/{r.Sample_ID}.adpc.bin.numSnps.txt"
        )
...
def test_contamination_test(tmp_path, conda_envs):
    ...
    data_store = (
        ...
        .add_reference_files(copy=False)
        .add_user_files(entry_point="gtc", copy=False)
        .copy("production_outputs/contam", "sample_contamination/convert_gtc_to_illumina_adpc")
        .copy(
            "production_outputs/GSAMD-24v1-0_20011747_A1.AF.abf.txt",
            "sample_contamination/GSAMD-24v1-0_20011747_A1.AF.abf.txt",
        )
        .make_config()
        .make_snakefile(
            """
            ...
            rule all:
                input:
                    cfg.expand("sample_contamination/contamination_test/{Sample_ID}.contam.out")
            """
        )
    )
    ...
    # sample.
    for r in data_store.ss.data.itertuples(index=False):
        obs_file = tmp_path / f"sample_contamination/contamination_test/{r.Sample_ID}.contam.out"
        exp_file = (
...
def test_agg_contamination_test(tmp_path, conda_envs):
    ...
    data_store = (
        ...
        .add_user_files(entry_point="gtc", copy=False)
        .copy(
            "production_outputs/one_samp_b_1000g_contam", "sample_contamination/contamination_test"
        )
        .copy(
    ...
    obs_file = tmp_path / "sample_contamination/agg_contamination_test.csv"
